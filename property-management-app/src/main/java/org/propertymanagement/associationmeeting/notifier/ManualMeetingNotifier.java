...
public class ManualMeetingNotifier implements MeetingNotification {
    ...
    public ManualMeetingNotifier(NotificationManager notificationManager, MeetingRepository meetingRepository) {
    ...
    public void notifyForApproval(MeetingInvite invite) {
        NeighbourgId approverId = new NeighbourgId(invite.approverId().value());

        ScheduledAssociationMeeting scheduledAssociationMeeting = meetingRepository.fetchScheduledAssociationMeeting(invite.communityId(), invite.trackerId());
        Participant president = scheduledAssociationMeeting.participants().stream()
                .filter(participant -> participant.role() == Participant.ParticipantRole.PRESIDENT && participant.id().equals(invite.approverId()))
                .findFirst()
                .orElseThrow(() -> {
                    String message = "President approver not found Id=" + approverId.value() + " CommunityId=" + invite.communityId().value();
    ...
    private Meeting newMeeting(MeetingInvite invite) {
        ...
        description.append("Yearly Community Association Meeting\n\n")
                .append("CommunityId: ").append(invite.communityId().value()).append("\n")
                .append("ApproverId: ").append(invite.approverId().value()).append("\n")
                .append("TrackerId: ").append(invite.trackerId().value());
